...
function UserDashboard() {
  ...
  const [similarEventsPackage, setSimilarEventsPackage] = useState(null);
  ...
  const loadEvents = async () => {
    try {
      const res = await fetch(apiUrl("/api/events/my"), { headers: headers() });
  ...
  const loadNotifications = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/notifications"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadTickets = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/support"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadPayments = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/payments"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadFaq = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/faq"));
      const data = await parseJsonResponse(res);
  ...
  const loadMeta = async () => {
    try {
      const res = await fetch(apiUrl("/api/events/meta"));
      const data = await parseJsonResponse(res);
  ...
  const loadSuggestions = async () => {
    try {
      const res = await fetch(apiUrl(`/api/events/suggestions?guestCount=${form.guestCount || 10}`), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
      if (res.ok) {
  ...
  const loadConversations = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/conversations"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadConvMessages = async (convId) => {
    if (!convId) return;
    try {
      const res = await fetch(apiUrl(`/api/user/conversations/${convId}/messages`), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const openConversationForEvent = async (eventId) => {
    try {
      const res = await fetch(apiUrl(`/api/user/conversations/for-event/${eventId}`), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const sendConvMessage = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/user/conversations/${selectedConv._id}/messages`), {
        method: "POST",
        headers: json(),
  ...
  const loadManagerRequest = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/manager-request"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const requestManagerRole = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/request-manager"), { method: "POST", headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const applySuggestion = (s) => {
    ...
    setSection("book");
    setSimilarEventsPackage(null);
  };

  const openSimilarEvents = async (s) => {
    setSimilarEventsPackage(s);
    setSimilarEventsList([]);
    setSimilarEventsLoading(true);
    try {
      const res = await fetch(apiUrl(`/api/events/similar?type=${encodeURIComponent(s.type)}`), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
      if (res.ok) setSimilarEventsList(data.events || []);
      else setSimilarEventsList([]);
    } catch {
      setSimilarEventsList([]);
    } finally {
  ...
  const handleCreate = async (e) => {
    e.preventDefault();
    setError("");
    const required = { type: form.type, title: form.title, scheduledAt: form.scheduledAt, addressLine: form.addressLine, city: form.city, pincode: form.pincode };
  ...
  const cancelEvent = async (id) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/events/${id}/cancel`), { method: "POST", headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const rescheduleEvent = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/events/${rescheduleId}/reschedule`), {
        method: "POST",
        headers: json(),
  ...
  const submitFeedback = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/user/events/${feedbackEvent._id}/feedback`), {
        method: "POST",
        headers: json(),
  ...
  const downloadInvoice = async (eventId) => {
    try {
      const url = apiUrl(`/api/user/invoice/${eventId}?format=pdf`);
  ...
  const submitSupport = async (e) => {
    ...
    setError("");
    try {
      const res = await fetch(apiUrl("/api/user/support"), {
        method: "POST",
        headers: json(),
  ...
  const markNotificationRead = async (id) => {
    try {
      await fetch(apiUrl(`/api/user/notifications/${id}/read`), { method: "PATCH", headers: headers() });
      loadNotifications();
    } catch {
      /* optional background fetch */
    }
  ...
  const enable2FA = async () => {
    try {
      const res = await fetch(apiUrl("/api/auth/2fa/enable"), { method: "POST", headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
      if (!res.ok) throw new Error(data.message || "Failed");
      setTwoFactorQr(data);
  ...
  const loadSurveyQuestions = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/survey/questions"), { headers: headers() });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const submitSurvey = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/user/events/${surveyEvent._id}/survey`), {
        method: "POST",
        headers: json(),
  ...
  return (
    <div className="dashboard admin-dashboard">
        ...
        {section === "dashboard" && (
          <>
            ...
            {suggestions.length > 0 && (
              <section className="section suggested-packages">
                ...
                <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", marginBottom: 16 }}>
                  Every event is <strong>customizable</strong> to your needs — pick a package below and we’ll tailor it for you.
                </p>
                <div className="suggestion-cards">
                  {suggestions.map((s, i) => (
                    <div key={i} className="suggestion-card">
                      <div className="suggestion-card-image-wrap">
                        <img src={s.imageUrl || `https://picsum.photos/seed/${s.type}/400/260`} alt={s.label} className="suggestion-card-image" />
                        <span className="suggestion-card-badge">Customizable</span>
                      </div>
                      <div className="suggestion-card-body">
                        <h3 className="suggestion-card-title">{s.label}</h3>
                        {s.theme && <p className="suggestion-card-meta">Theme: {s.theme}</p>}
                        {s.venueType && <p className="suggestion-card-meta">Venue: {s.venueType}</p>}
                        {s.description && <p className="suggestion-card-desc">{s.description}</p>}
                        {s.services?.length > 0 && (
                          <p className="suggestion-card-services">Includes: {s.services.map((sv) => sv.replace(/_/g, " ")).join(", ")}</p>
                        )}
                        {(s.pastEvents?.length ?? 0) > 0 && (
                          <div className="suggestion-past-events">
                            <h4 className="suggestion-past-title">How others booked</h4>
                            <ul className="suggestion-past-list">
                              {s.pastEvents.slice(0, 4).map((ev, j) => (
                                <li key={j} className="suggestion-past-item">
                                  {ev.imageUrl ? <img src={ev.imageUrl} alt="" className="suggestion-past-thumb-img" /> : <span className="suggestion-past-thumb" aria-hidden />}
                                  <span><strong>{ev.title}</strong> · {ev.guestCount || "—"} guests · {ev.venue || "—"} · {ev.performedAt || ev.scheduledAt ? new Date(ev.performedAt || ev.scheduledAt).toLocaleDateString() : ""}</span>
                                </li>
                              ))}
                            </ul>
                          </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          <button type="button" className="btn btn-secondary btn-sm" onClick={() => openSimilarEvents(s)}>
                            See all similar events
                          </button>
                          <button type="button" className="btn btn-primary suggestion-card-btn" onClick={() => applySuggestion(s)}>
                            Use this package & book
                          </button>
                        </div>
                      </div>
                    </div>
                ...
                {suggestionMeta?.venueTypes?.length > 0 && (
                  <p style={{ color: "var(--text-muted)", fontSize: "0.85rem", marginTop: 16 }}>Venue types: {suggestionMeta.venueTypes.join(", ")}</p>
                )}
              </section>
            )}
          </>
      ...
      {similarEventsPackage && (
        <Modal open={!!similarEventsPackage} onClose={() => setSimilarEventsPackage(null)} title={`Similar events: ${similarEventsPackage.label}`}>
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem", marginBottom: 12 }}>Past bookings and events from managers in this category. Every event is customizable to you.</p>
          {similarEventsLoading ? (
            <p>Loading…</p>
          ) : similarEventsList.length === 0 ? (
            <p style={{ color: "var(--text-muted)" }}>No similar events in this category yet. Be the first to book!</p>
          ) : (
            <ul className="similar-events-list">
              {similarEventsList.map((ev, j) => (
                <li key={j} className="similar-events-item">
                  {ev.imageUrl ? <img src={ev.imageUrl} alt="" className="similar-events-thumb" /> : <span className="similar-events-thumb-placeholder" />}
                  <div>
                    <strong>{ev.title}</strong>
                    <span className="similar-events-meta"> · {ev.guestCount || "—"} guests · {ev.venue || "—"} · {ev.date ? new Date(ev.date).toLocaleDateString() : ""} {ev.source === "portfolio" ? "(manager)" : ""}</span>
                    {ev.description && <p className="similar-events-desc">{ev.description}</p>}
                  </div>
                </li>
              ))}
            </ul>
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
            <button type="button" className="btn btn-primary" onClick={() => applySuggestion(similarEventsPackage)}>Use this package & book</button>
            <button type="button" className="btn btn-secondary" onClick={() => setSimilarEventsPackage(null)}>Close</button>
          </div>
        </Modal>
      )}
