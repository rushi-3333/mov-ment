...
function AdminDashboard() {
  ...
  const fetchAll = async () => {
    try {
      ...
      const uRes = await fetch(apiUrl("/api/admin/users"), { headers });
      if (checkAuth(uRes)) return;
      const uData = await parseJsonResponse(uRes);
      if (uRes.status === 403) {
        setError((uData && uData.message) ? uData.message : "Access denied. Please sign out and sign in with the owner account (admin@gmail.com) or an admin account.");
        setModalOpen(true);
        setShowRelogin(true);
        return;
      }
      ...
      const [pRes, mRes, eRes] = await Promise.all([
        fetch(apiUrl("/api/admin/pending-managers"), { headers }),
        fetch(apiUrl("/api/admin/managers"), { headers }),
  ...
  const loadConversations = async () => {
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const res = await fetch(apiUrl("/api/admin/conversations"), { headers });
  ...
  const loadAdminConvMessages = async (convId) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      const res = await fetch(apiUrl(`/api/admin/conversations/${convId}/messages`), { headers });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadSupportTickets = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/support-tickets"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadRefunds = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/refunds"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadUserActivity = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/user-activity?limit=100"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadAnalytics = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/analytics/dashboard"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadReports = async (period) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/analytics/reports?period=${period || "month"}`), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadManagerPerf = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/analytics/manager-performance"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const loadLoadBalancing = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/analytics/load"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  useEffect(() => {
    ...
    if (activeSection === "analytics") { loadAnalytics(); loadReports("month"); loadManagerPerf(); loadLoadBalancing(); }
  ...
  const approveManager = async (id) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/managers/${id}/approve`), {
        method: "POST",
  ...
  const promoteToAdmin = async (id) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/users/${id}/promote-admin`), {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (checkAuth(res)) return;
  ...
  const removeManager = async (id) => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/admin/managers/${id}/remove`), {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (checkAuth(res)) return;
  ...
  const submitSetPassword = async () => {
    if (!setPasswordUser || !setPasswordValue || setPasswordValue.length < 6) {
    ...
    try {
      const res = await fetch(apiUrl(`/api/admin/users/${setPasswordUser._id}/set-password`), {
        method: "POST",
      ...
      closeSetPassword();
      window.alert(data.message || "Password updated. User can sign in with the new password.");
    } catch (err) {
      setError(err.message);
      setModalOpen(true);
    } finally {
  ...
  const loadManagerRequests = async () => {
    try {
      const res = await fetch(apiUrl("/api/admin/manager-requests"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
  ...
  const approveManagerRequest = async (id) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/manager-requests/${id}/approve`), {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (checkAuth(res)) return;
  ...
  const rejectManagerRequest = async (id) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/manager-requests/${id}/reject`), {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
      if (checkAuth(res)) return;
  ...
  const loadProfile = async () => {
    try {
      const res = await fetch(apiUrl("/api/user/profile"), { headers: { Authorization: `Bearer ${token}` } });
      if (checkAuth(res)) return;
      const data = await parseJsonResponse(res);
      if (res.ok) {
        setProfile(data);
  ...
  const processRefund = async (id, status) => {
    try {
      const res = await fetch(apiUrl(`/api/admin/refunds/${id}`), {
        method: "PATCH",
        headers: { ...json(), Authorization: `Bearer ${token}` },
  ...
  const saveAssignTeam = async () => {
    ...
    try {
      const res = await fetch(apiUrl(`/api/admin/events/${assignTeamEvent._id}/assign-team`), {
        method: "POST",
        headers: { ...json(), Authorization: `Bearer ${token}` },
    ...
    <div className="dashboard admin-dashboard">
      ...
      <div className="admin-main">
        ...
        {activeSection === "users" && (
          <section className="section">
            ...
            {usersExcludingManagers.length === 0 ? (
              <p style={{ color: "var(--text-muted)" }}>No users to show. Users will appear here when they register (managers appear under Managers).</p>
            ) : (
                  ...
                  <tbody>
                    {usersExcludingManagers.map((u) => (
                      <tr key={u._id || u.email}>
                        ...
                        <td>
                          <button type="button" className="btn btn-secondary btn-sm" onClick={() => openSetPassword(u)} title="Set a new password for this user (e.g. if they forgot it)">
                            Set password
                          </button>
                        </td>
                      </tr>
                ...
                <tbody>
                  {managers.map((m) => (
                    <tr key={m._id}>
                      ...
                      <td>{m.location?.city || "—"}</td>
                      <td>
                        <button type="button" className="btn btn-secondary btn-sm" onClick={() => openSetPassword(m)} title="Set a new password for this manager">
                          Set password
                        </button>
                        {m.approved && (
                          <button type="button" className="btn btn-secondary btn-sm" style={{ marginLeft: 8 }} onClick={() => promoteToAdmin(m._id)}>
                            Make admin
                          </button>
                        )}
                        <button type="button" className="btn btn-danger btn-sm" style={{ marginLeft: 8 }} onClick={() => removeManager(m._id)}>
          ...
          <section className="section">
            ...
            {reports && (
              <div style={{ marginBottom: 24 }}>
                <h4 style={{ marginBottom: 8 }}>Period report ({reports.period || "month"})</h4>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12 }}>
                  <div style={{ padding: 12, background: "var(--bg-input)", borderRadius: 8, border: "1px solid var(--border)" }}>
                    <div style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>Revenue</div>
                    <div style={{ fontSize: "1.25rem", fontWeight: 700 }}>₹{reports.revenue ?? 0}</div>
                  </div>
                  <div style={{ padding: 12, background: "var(--bg-input)", borderRadius: 8, border: "1px solid var(--border)" }}>
                    <div style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>Bookings</div>
                    <div style={{ fontSize: "1.25rem", fontWeight: 700 }}>{reports.bookings ?? 0}</div>
                  </div>
                  <div style={{ padding: 12, background: "var(--bg-input)", borderRadius: 8, border: "1px solid var(--border)" }}>
                    <div style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>Cancelled</div>
                    <div style={{ fontSize: "1.25rem", fontWeight: 700 }}>{reports.cancelled ?? 0}</div>
                  </div>
                  <div style={{ padding: 12, background: "var(--bg-input)", borderRadius: 8, border: "1px solid var(--border)" }}>
                    <div style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>Avg rating</div>
                    <div style={{ fontSize: "1.25rem", fontWeight: 700 }}>{reports.avgRating ?? 0} ★</div>
                  </div>
                </div>
              </div>
            )}
      ...
      {setPasswordUser && (
        <Modal
          ...
          title="Set password"
        >
          <p style={{ marginBottom: 12 }}>Set a new password for <strong>{setPasswordUser.name}</strong> ({setPasswordUser.email}). They can sign in with this password immediately.</p>
          <div className="form-group">
            <label htmlFor="admin-set-password">New password (min 6 characters)</label>
            <input
          ...
          <div style={{ marginTop: 16, display: "flex", gap: 8 }}>
            <button type="button" className="btn btn-primary" onClick={submitSetPassword} disabled={setPasswordLoading || setPasswordValue.length < 6}>
              {setPasswordLoading ? "Saving…" : "Set password"}
            </button>
            <button type="button" className="btn btn-secondary" onClick={closeSetPassword} disabled={setPasswordLoading}>Cancel</button>
          </div>
        </Modal>
      )}
