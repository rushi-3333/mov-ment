...
// Manager's performed/done events (uploaded to show in that category to users)
router.get("/performed-events", managerAuth, async (req, res) => {
  try {
    const mongoose = require("mongoose");
    if (mongoose.connection.readyState !== 1) {
      return res.status(503).json({ message: "Database connecting or unavailable. Try again in a moment." });
    }
    const list = await ManagerPortfolioEvent.find({ manager: req.user.id }).sort({ performedAt: -1 }).lean();
    return res.json(list);
  } catch (err) {
    console.error("GET /performed-events error:", err.message || err);
    return res.status(500).json({ message: "Server error", detail: err.message });
  }
});

router.post("/performed-events", managerAuth, async (req, res) => {
  try {
    const { type, title, description, imageUrl, venue, guestCount, performedAt } = req.body;
    ...
    const doc = await ManagerPortfolioEvent.create({
      manager: req.user.id,
      type,
    ...
    console.error("POST /performed-events error:", err.message || err);
    return res.status(500).json({ message: "Server error", detail: err.message });
  }
});

router.delete("/performed-events/:id", managerAuth, async (req, res) => {
  try {
    const doc = await ManagerPortfolioEvent.findOne({ _id: req.params.id, manager: req.user.id });
    if (!doc) return res.status(404).json({ message: "Not found" });
    await ManagerPortfolioEvent.deleteOne({ _id: req.params.id });
    return res.json({ message: "Deleted" });
  } catch (err) {
    return res.status(500).json({ message: "Server error" });
  }
