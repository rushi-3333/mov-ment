...
// All users (everyone in DB; frontend may filter by role)
router.get("/users", auth(), async (req, res) => {
  try {
    const current = await requireAdminOrOwner(req, res);
    if (!current) return;
    const list = await User.find().select("-passwordHash -twoFactorSecret").lean();
...
// Set a user's password (admin/owner only). Use when a user forgets password or needs a reset.
router.post("/users/:id/set-password", auth(), async (req, res) => {
  try {
    const current = await requireAdminOrOwner(req, res);
    if (!current) return;
    ...
    await target.save();
    return res.json({ message: "Password updated. User can sign in with the new password." });
  } catch (err) {
